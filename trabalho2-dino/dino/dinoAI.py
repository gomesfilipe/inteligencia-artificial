...
def PSO(maxIter):
    ...
    for i in range(maxIter):
        ...
        for index, particle in enumerate(population):
            global aiPlayer
            aiPlayer = DecisionTreeKeyClassifier(particle)
            res, value = manyPlaysResults(3)

            # Checando se é a melhor posição encontrada pela partícula
            if i == 0:
                bestLocals.append(particle)
                maxLocalValues.append(value)

            elif value > maxLocalValues[index]:
                bestLocals[index] = particle
                maxLocalValues[index] = value

            # Checando se é a melhor posição encontrada por todo o enxame de partículas
            if value > maxGlobalValue:
                bestGlobal = particle
                maxGlobalValue = value

        # print(f'Best value: {maxGlobalValue}')
        # print(bestGlobal)
